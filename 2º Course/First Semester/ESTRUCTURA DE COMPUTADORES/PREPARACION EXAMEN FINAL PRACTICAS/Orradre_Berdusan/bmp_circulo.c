...
void pixels_generator_circulo(unsigned int origen_x, unsigned int origen_y, unsigned int radio_dimension,unsigned int dimension,RGB_data reg_mem[][DIMENSION]);
...
void main(void)
{
    ...
    pixels_generator_circulo(ORIGEN_X,ORIGEN_Y,RADIO_DIMENSION,DIMENSION,buffer);
