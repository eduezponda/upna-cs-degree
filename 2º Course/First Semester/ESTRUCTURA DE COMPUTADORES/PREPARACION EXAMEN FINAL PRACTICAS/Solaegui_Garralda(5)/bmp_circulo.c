/*
Programa:     	bmp_circulo.c -> como se pasa la dimension de un array como argumento de una funcion
Descripción:  	Genera un rectángulo en una imagen bitmap en formato BMP y la guarda en el fichero test.bmp.  
Funciones:    	void pixels_generator_circulo(unsigned int origen_x, unsigned int origen_y, unsigned int lado, unsigned int proporcion, unsigned int dimension, RGB_data reg_mem[][DIMENSION]); Definida en el módulo pixel_funcion_arguDimension.c ó pixel_funcion_arguDimension.s
...
Compilación: generar el módulo objeto pixel_circulo.o  : gcc -m32 -g -c -o pixel_circulo.o pixel_circulo.c ó as --32 --gstabs -o pixel_circulo.o pixel_circulo.s
             gcc -g -m32 -o bmp_circulo bmp_circulo.c pixel_circulo.o
Ejecución:    $./bmp
...
/*
#define DIMENSION   1024  // OJO _también asignado el valor en la  definición de funcion_mod.c pero no en funcion_mod.s
#define RADIO (DIMENSION-2)/2
#define ORIGEN_X (DIMENSION-2)/2
#define ORIGEN_Y (DIMENSION-2)/2
#define PROPORCION 0xFF
*/
...
void pixels_generator_circulo(unsigned int origen_x, unsigned int origen_y, unsigned int lado, unsigned int proporcion, unsigned int dimension, RGB_data reg_mem[][DIMENSION]);
...
int main(void)
{
    ...
    pixels_generator_circulo(ORIGEN_X,ORIGEN_Y,RADIO,PROPORCION,DIMENSION,buffer);
