...
int grupo_de_sockets_acepta_nuevo_cliente(grupo_de_sockets *grupo, int max) {
    ...
    if ( ! grupo_de_sockets_guarda_socket(grupo,cli_sock,cli_stream, max) ) {
        printf("Nueva conexi�n denegada, n�mero m�ximo de clientes alcanzado\n");
        fprintf(cli_stream, "N� MAX CLIENTES ALCANZADO\n");
        fclose(cli_stream);
        return 0;
    }
    if (cli_sock) {
		printf("Conexi�n establecida desde la IP %s puerto %d\n", inet_ntoa(cli_dir.sin_addr), ntohs(cli_dir.sin_port));
	}
	else {
