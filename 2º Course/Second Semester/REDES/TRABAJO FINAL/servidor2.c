...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        for (int i = 0; i<max; i++) {
            ...
            if ( pollinfo->revents ) {
                int res = opciones(&sockets, tab, i, max, &partidaEnCurso, filas, columnas, true);
                if ( res == 0 ) {
       ...
       if (!partidaEnCurso && sockets.nclients == 1 && sockets.cli[0].nameset) {
			partidaEnCurso = inicioPartida(&sockets, columnas, filas, true);
	   }
   }
}
